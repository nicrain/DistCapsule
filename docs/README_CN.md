...
*   **双重生物识别**: 同时支持 **指纹识别** (DY-50) 和 **人脸识别** (Camera Module 3)。
*   **智能节能 (Eco Mode)**: 30秒无操作自动熄灭屏幕背光，触摸指纹传感器或检测到人脸瞬间唤醒。
*   **实时时钟**: 待机界面显示动态更新的系统时间。
*   **可视化进度条**: 开锁倒计时采用直观的进度条显示。
*   **精确权限控制**: 认证后，普通用户解锁专属舵机；管理员仅显示欢迎界面，用于维护/管理。
...
*   **2025-12 (S5)**: 
    *   **人脸识别集成**: 增加对 Pi 5 Camera Module 3 的支持，优化 GStreamer 管道。
    *   **项目重构**: 整理目录结构，将驱动移入 `hardware/`，工具移入 `tools/`。
    *   **硬件兼容**: 舵机改用软件 PWM (`lgpio`) 以避免与 Pi 5 风扇冲突。
