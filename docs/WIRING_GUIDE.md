# 🔌 硬件接线指南 (Hardware Wiring Guide)

本文档详细记录了 **智能胶囊分配器** 项目中所有硬件模块与 Raspberry Pi 5 的连接方式。

## Raspberry Pi 5 GPIO 引脚图 (Pinout Diagram)

![Raspberry Pi 5 GPIO Pinout](https://www.raspberrypi.com/documentation/computers/images/GPIO-Pinout-Diagram-2.png?hash=df7d7847c57a1ca6d5b2617695de6d46)

---

**最后更新日期：** 2025-12-31
**主控板：** Raspberry Pi 5
**控制库：** `lgpio` (软件模拟 PWM)

---

## 1. 舵机模块 (Servo Motor)
**型号：** SG90 (Micro Servo 9g) x 5
**供电要求：** 4.8V - 6V。
**注意：** 本项目代码使用 `lgpio` 库实现软件 PWM 控制。这意味着你可以使用任意 GPIO 引脚控制舵机，不再受限于树莓派的 4 个硬件 PWM 通道。

**⚠️ 电源警告：** 请务必使用 **外部 5V 电源** 为所有舵机供电，并与 Pi **共地 (Connect Grounds)**。切勿直接使用 Pi 的 5V 引脚驱动 5 个舵机。

| 舵机编号 | 信号线 (橙) | BCM GPIO | 物理引脚 (Pin) | 备注 |
| :--- | :--- | :--- | :--- | :--- |
| **Servo 1** | **GPIO 18** | GPIO 18 | Pin 12 | 亦支持硬件 PWM |
| **Servo 2** | **GPIO 6**  | GPIO 6  | Pin 31 | 纯软件 PWM |
| **Servo 3** | **GPIO 12** | GPIO 12 | Pin 32 | 亦支持硬件 PWM |
| **Servo 4** | **GPIO 13** | GPIO 13 | Pin 33 | 亦支持硬件 PWM |
| **Servo 5** | **GPIO 19** | GPIO 19 | Pin 35 | 亦支持硬件 PWM |

| 通用线色 | 功能 | 连接目标 |
| :--- | :--- | :--- |
| 🔴 **红** | VCC (+) | **外部 5V 电源** |
| 🟤 **棕** | GND (-) | **GND** (Pi Pin 6/9/14... 与外部电源共地) |

> **配置说明：**
> 由于代码使用的是 `lgpio` 直接控制 GPIO，**不需要** 在 `/boot/firmware/config.txt` 中开启硬件 PWM (`dtoverlay=pwm...`)。保持默认 GPIO 模式即可。

---

## 2. 显示屏 (IPS 1.3" ST7789)
**型号：** ZJY-IPS130-V2.0 (240x240)
**接口：** SPI (7-Pin)

| 屏幕引脚 | 功能 | 树莓派引脚 (Physical) | BCM (GPIO) | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| **GND** | 地 | **Pin 20** | GND | 共地 |
| **VCC** | 电源 | **Pin 17** | 3.3V | **注意接 3.3V** |
| **SCL** | SPI 时钟 | **Pin 23** | GPIO 11 (SCLK) | SPI0 SCLK |
| **SDA** | SPI 数据 | **Pin 19** | GPIO 10 (MOSI) | SPI0 MOSI |
| **RES** | 复位 | **Pin 22** | GPIO 25 | 复位信号 |
| **DC** | 数据/命令 | **Pin 18** | GPIO 24 | Data/Command |
| **BLK** | 背光 | **Pin 13** | GPIO 27 | 高电平亮 |

> **注意**: 此屏幕没有 CS 引脚，直接使用 SPI0 即可。

---

### 3. 指纹模块 (DY-50 / R307)
**通信协议**: UART (Serial)
**关键注意**:
*   **交叉连接**: 传感器的 TX 接树莓派的 RX，传感器的 RX 接树莓派的 TX。
*   **端口选择 (Pi 5)**: 在 Raspberry Pi 5 上，GPIO 14/15 对应的硬件串口是 **`/dev/ttyAMA0`**。

| 传感器引脚 | 树莓派引脚 (Physical) | 树莓派 GPIO | 功能 |
| :--- | :--- | :--- | :--- |
| VCC (红) | Pin 1 | 3.3V Power | 供电 (3.3V) |
| GND (黑) | Pin 6 | Ground | 接地 |
| TX (绿/黄) | Pin 10 | GPIO 15 (RXD) | 数据接收 |
| RX (白/蓝) | Pin 8 | GPIO 14 (TXD) | 数据发送 |
| WAK (选) | - | - | 唤醒 (暂不使用) |
| VT (选) | - | - | 触摸感应 (暂不使用) |

---

## 5. 摄像头模块 (Camera Module 3)
**型号：** Raspberry Pi Camera Module 3 (IMX708)
**接口：** CSI (22-pin Mini CSI on Pi 5)

**⚠️ 连接注意 (Pi 5 专用)：**
*   **排线方向**：在树莓派端，排线的**金属触点（银色面）必须朝向 PCB 主板**（即朝下），**蓝色/黑色绝缘面朝上**（朝向卡扣）。
*   **接口选择**：可以使用 `CAM/DISP 0` 或 `CAM/DISP 1`。推荐使用 `CAM/DISP 1` (靠近 USB 接口一侧)，或者 `CAM 0` (靠近电源)。
*   **排线规格**：Pi 5 使用更窄的 22-pin 排线 (Mini CSI)，与 Pi 3/4 的标准 CSI 不通用。

**测试命令：**
```bash
rpicam-hello -t 5000
```
如果你看到 "Fixed dependency cycle" 或 Error -5，请首先检查排线是否插反。

---

## 7. 唤醒按钮 (Wake-Up Button)
**模块**: DFRobot Digital Push Button (DFR0029-B) 或通用点动开关。
**用途**: 从低功耗休眠模式中唤醒系统（点亮屏幕、启动摄像头/指纹）。

| DFRobot 线色 | 功能 | 树莓派引脚 | 物理引脚 (Board) |
| :--- | :--- | :--- | :--- |
| **黑 (Black)** | GND | **GND** | Pin 6, 9, 14... |
| **红 (Red)** | VCC | **3.3V** | Pin 1 |
| **绿 (Green)** | Signal | **GPIO 26** | Pin 37 |

> **注意**: 请务必连接 **3.3V** 而不是 5V，以保护 Pi 的 GPIO 引脚。

---

## 8. 软件配置检查
### 8.1 启用串口 (指纹)
确保 Raspberry Pi 的串口未被控制台占用：
1. 运行 `sudo raspi-config`
2. 选择 `Interface Options` -> `Serial Port`
3. **Login shell:** 选择 **No**
4. **Serial port hardware:** 选择 **Yes**
5. 重启生效。

### 4.2 Python 依赖
确保安装了必要的库：
```bash
sudo apt install python3-lgpio python3-serial python3-pil python3-rpi.gpio
pip3 install adafruit-circuitpython-fingerprint st7789
```
